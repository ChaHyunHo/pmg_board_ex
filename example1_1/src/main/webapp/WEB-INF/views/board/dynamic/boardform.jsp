 ...
 <!-- Begin Page Content -->
      ...
      <div class="card shadow mb-4">
        ...
        <div>
        	<div style="float: left; margin-bottom: 10px;"> 
        		...
        		<!-- Single button -->
				<div class="btn-group">
					<div style="margin-top: 5px; margin-right: 3px; margin-left: 4px;"><b>show:</b></div>
					 <select id="perPageBtn" name="dataTable_length" style="height: 38px;" 
        	...
        	<div style="float: right; margin-bottom: 10px;" >
				...
				<div class="input-group">
				 ...
				 class="custom-select-sm form-control form-control-sm">
					...
					<input type="text" class="form-control" placeholder="Search..." id="searchTitle" autocomplete="new-password">
            ...
            <table class="table" id="dataTable" width="100%" cellspacing="0">
              <thead>
                <tr>
                  ...
                  <th>제목</th>
                  <th>작성자</th>
                  <th id="thOrder"><a data-order="R" id="orderBtn" href="javascript:void(0);" style="color: #858796; text-decoration: none;">
	                  	작성일
	                  	<c:if test="${orderVal eq 'RA'}">
	                  		<i class="fas fa-long-arrow-alt-up" id="r_up" style="margin-left: 5px; color: orange;"></i>
	                  	</c:if>
	                  	<c:if test="${orderVal ne 'RA'}">
	                  		<i class="fas fa-long-arrow-alt-up" id="r_up" style="margin-left: 5px;"></i>
	                  	</c:if>
	                  	<c:if test="${orderVal eq 'RD'}">
	                  		<i class="fas fa-long-arrow-alt-down" id="r_down" style="color: orange;"></i>
	                  	</c:if>
	                  	<c:if test="${orderVal ne 'RD'}">
	                  		<i class="fas fa-long-arrow-alt-down" id="r_down" ></i>
	                  	</c:if>
                  	</a>
                  </th>
                  <th id="thOrder"><a data-order="U" id="orderBtn" href="javascript:void(0);" style="color: #858796; text-decoration: none;">
	                  	수정일
                  		<c:if test="${orderVal eq 'UA'}">
                  		<i class="fas fa-long-arrow-alt-up" id="r_up" style="margin-left: 5px; color: orange;"></i>
	                  	</c:if>
	                  	<c:if test="${orderVal ne 'UA'}">
	                  		<i class="fas fa-long-arrow-alt-up" id="r_up" style="margin-left: 5px;"></i>
	                  	</c:if>
	                  	<c:if test="${orderVal eq 'UD'}">
	                  		<i class="fas fa-long-arrow-alt-down" id="r_down" style="color: orange;"></i>
	                  	</c:if>
	                  	<c:if test="${orderVal ne 'UD'}">
	                  		<i class="fas fa-long-arrow-alt-down" id="r_down" ></i>
	                  	</c:if>
                  	</a>
                  </th>
                   <th id="thOrder"><a data-order="V" id="orderBtn" href="javascript:void(0);" style="color: #858796; text-decoration: none;">
                  	조회수
                  		<c:if test="${orderVal eq 'VA'}">
	                  		<i class="fas fa-long-arrow-alt-up" id="r_up" style="margin-left: 5px; color: orange;"></i>
	                  	</c:if>
	                  	<c:if test="${orderVal ne 'VA'}">
	                  		<i class="fas fa-long-arrow-alt-up" id="r_up" style="margin-left: 5px;"></i>
	                  	</c:if>
	                  	<c:if test="${orderVal eq 'VD'}">
	                  		<i class="fas fa-long-arrow-alt-down" id="r_down" style="color: orange;"></i>
	                  	</c:if>
	                  	<c:if test="${orderVal ne 'VD'}">
	                  		<i class="fas fa-long-arrow-alt-down" id="r_down" ></i>
	                  	</c:if>
                 	</a>
                 </th>
              ...
              <tbody>
              			...
              			<c:if test="${pageDTO.pagevo.page eq '1'}">
              				<c:if test="${empty searchTitle}">
		              			 <tr id="board_tr" style="font-weight: 800;">
			                   	   ...
			                   	   <td style="width: 500px;"><a href="javascript:void(0);" id="board_title" style="color: #858796;"><c:out value="${notice.title}" escapeXml="false" /></a></td>
				                   ...
				                   <td style="width: 100px; padding-left: 20px;"><span class="badge badge-pill badge-light"><c:out value="${notice.viewCount}" escapeXml="false"  /></span></td>
				                 </tr>
				             </c:if>
		                </c:if>
              		</c:forEach>
            	  	...
            	  	<c:forEach items="${boardList}" var="list">
            	  		   <tr id="board_tr">
           		   		   ...
           		   		   <td style="width: 500px;"><a href="javascript:void(0);" id="board_title" style="color: #858796;">
           		   		   	<c:out value="${list.title}" escapeXml="false" /></a>
           		   		   		<c:if test="${list.newLabel}">
		                   ...
		                   <td style="width: 100px; padding-left: 20px;"><span class="badge badge-pill badge-light"><c:out value="${list.viewCount}" escapeXml="false"  /></span></td>
		                 </tr>
             ...
             <c:if test="${empty boardList}">
              	<div style="text-align: center; margin-top: 30px; margin-right: 20px;"> 
              		게시글이 존재하지 않습니다.
              	</div>
              </c:if>
              <c:if test="${!empty boardList}">
              		...
              		<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-center pagination-sm custompagination">
						  <c:if test="${pageDTO.prev}">
						  	<li class="page-item">
							<a class="page-link" href="javascript:void(0);"  aria-label="Previous" id="prevBtn">
						  ...
						  <c:forEach begin="${pageDTO.startPage}" end="${pageDTO.endPage}" var="index">
						  	<c:if test="${pageDTO.endPage ne 1}">
						  		<li class="page-item <c:out value="${pageDTO.pagevo.page == index?'active' : '' }"/>">
							  		<a class="page-link"  href="javascript:void(0);" id="pageIndex">${index}</a>
							  	</li>
						  	</c:if>
						  </c:forEach>
						  ...
						  <c:if test="${pageDTO.next && pageDTO.endPage > 0}">
						  	<li class="page-item">
							<a class="page-link" href="javascript:void(0);" aria-label="Previous" id="nextBtn">
  ...
  <!-- End of Main Content -->
	...
	<form id="listForm">
      ...
      <input type="hidden" name="tempIdx" id="tempIdx" value="${tempIdx}" />
	</form>
